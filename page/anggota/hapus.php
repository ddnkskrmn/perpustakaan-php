...
if (empty($_GET['id'])) :
    ...
    if (isset($_GET['id'])) {
        ...
        $query = mysqli_query($connection, "DELETE FROM user WHERE id = '{$_GET['id']}'");
        if ($query) {
            echo '<div class="alert alert-success" role="alert">Hapus data berhasil !</div>';
            echo '<a href="index.php?page=anggota/index.php" class="btn btn-sm btn-outline-dark">Klik disini untuk kembali</a>';
        } else {
